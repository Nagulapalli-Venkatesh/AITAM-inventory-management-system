...
<h1 style="color:#2A1537;
  ...
  text-decoration-color: grey;">REQUESTED ORDERS</h1>
<h2></h2><br>
 ...
 <?PHP
            ...
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
                ...
                echo " <tr><th>".$row['sno']."</th><td>".$row['date']."</td><td>".$row['facid']."</td>
...
<h1 style="color:#2A1537;
  ...
  text-decoration:underline;
  text-decoration-style: 10px solid;
  text-decoration-color: grey;">INWARD REQUESTS</h1>
<h2></h2><br>
<span class="counter pull-right"></span>
...
<table class="table table-hover table-bordered results">
 ...
 <?PHP
            include 'connect.php';
            $oquery = "SELECT * FROM `inward` WHERE `status` = 'pending' ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
